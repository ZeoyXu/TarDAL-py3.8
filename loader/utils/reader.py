...
def ycbcr_read(img_path: Union[str, Path]) -> Tuple[Tensor, Tensor]:
...
def label_read(label_path: Union[str, Path]) -> Tensor:
...
def img_write(img_t: Tensor, img_path: Union[str, Path]):
    if img_t.shape[0] == 3:
...
def label_write(pred_i: Tensor, txt_path: Union[str, Path]):
