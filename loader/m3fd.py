...
class M3FD(Dataset):
    ...
    def __init__(self, root: Union[str, Path], mode: Literal['train', 'val', 'pred'], config: ConfigDict):
        super().__init__()
        root = Path(root)
        self.root = root
        self.mode = mode
    ...
    @staticmethod
    def pred_save(fus: Tensor, names: List[Union[str, Path]], shape: List[Size], pred: Optional[Tensor] = None, save_txt: bool = False):
        if pred is None:
            return M3FD.pred_save_no_boxes(fus, names, shape)
        return M3FD.pred_save_with_boxes(fus, names, shape, pred, save_txt)

    @staticmethod
    def pred_save_no_boxes(fus: Tensor, names: List[Union[str, Path]], shape: List[Size]):
        for img_t, img_p, img_s in zip(fus, names, shape):
            img_t = resize(img_t, img_s)
            img_write(img_t, img_p)

    @staticmethod
    def pred_save_with_boxes(fus: Tensor, names: List[Union[str, Path]], shape: List[Size], pred: Tensor, save_txt: bool = False):
