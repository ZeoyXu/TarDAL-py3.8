...
class InferF:
    def __init__(self, config: Union[Union[str, Path], ConfigDict], save_dir: Union[str, Path]):
        # init logger
        ...
        data_t = getattr(loader, config.dataset.name, None)  # dataset type

        self.data_t = data_t
        p_dataset = data_t(root=config.dataset.root, mode='val', config=config)

        # 此处的batch_size=1，这样所有图像才能正确输出
    ...
    def run(self):
        ...
        for sample in p_l:
            ...
            for name in sample['name']:
                self.data_t.pred_save(fus, [self.save_dir/name], shape=(606, 244))
                print(f"saved picture {name} in {self.save_dir/name}!!!")