...
class IQA:
    ...
    def inference(self, src: Union[str, Path], dst: Union[str, Path]):
        self.modality_inference(src, dst, 'ir')
        self.modality_inference(src, dst, 'vi')

    @torch.inference_mode()
    def modality_inference(self, src: Union[str, Path], dst: Union[str, Path], modality: Literal['ir', 'vi']):
        # create save folder
    ...
    @staticmethod
    def _imread(img_p: Union[str, Path]):
